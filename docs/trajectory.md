...
- **Entity Filtering & Config Toggles**: Uses [`ProjectileFilterKey`](../src/main/java/com/simonconrad/fireballpredictor/projectile/ProjectileFilterKey.java) and [ModConfig.java](../src/main/java/com/simonconrad/fireballpredictor/config/ModConfig.java) to toggle tracking for specific entity types (`trackFireballs`, `trackWitherSkulls`, `trackWindCharges`) and inferred owner categories (`BLAZE`, `GHAST`, `ENDER_DRAGON`, `WITHER`, `BREEZE`, `PLAYER`, `DISPENSER`, `COMMAND`).
- **Synchronous Single-Pass Execution**: Calculates complete predictions directly on the client tick (<0.05 ms total) without background thread hopping:
  - Performs the 200-tick flight path raycast, entity collision tests, 1,352-ray block destruction prediction (`ImpactPredictor.predictBrokenBlocks`), and dome mesh generation (`TrajectoryPredictor.createRenderData`) in a single pass.
  - Generates full `PredictionData` immediately on frame 0 with zero visual latency.
...
- **Deflection Tracking Preservation**: In `TrackedProjectile.updateInference`, if a fireball was previously deflected by a player, receiving authoritative server owner packets (`FireballOwnerPayload`) preserves `ProjectileOwner.PLAYER` and the deflector entity rather than reverting ownership to the original launcher mob.
- **Deduplicated Updates & Collision Caching**: Caches per-tick entity collision queries (`TrackedPrediction.getOrComputeDamageHit`) to eliminate redundant sweeps between `isThreateningPlayer` and the damage estimation overlay.
- **Optimized Path Rescans**: In `shouldRefresh`, evaluates $O(1)$ deflection and velocity drifts, checks the immediate next 5 ticks ahead for block placement on every tick, and throttles full-path rescans to once every 5 ticks to avoid redundant $O(N)$ block shape queries.
- **Collision Shape Emptiness & Dynamic Recalculation**: Tracks cached parameters (explosion power, `isDangerous()` state, owner attribution, block-breaking capability). Additionally, if the impacted block at `hitPos` has its collision shape emptied (`getCollisionShape(world, hitPos).isEmpty()`) — such as when a block is broken, replaced with fluids (water/lava), or substituted with non-solid blocks — the trajectory recalculates immediately rather than assuming only `isAir()`.
- **Inference Tracker Fallback Location Recording**: [`FireballInferenceTracker.recordFinalFireballLocation`](../src/main/java/com/simonconrad/fireballpredictor/client/network/FireballInferenceTracker.java) records terminal positions for explosion inference; for fireballs in long-distance flight (>3s) or untracked state, it safely falls back to resolving the entity owner and using the current entity position as `hitPos`.
