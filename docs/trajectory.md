...
- **Dynamic Recalculation Cache Invalidation**: Tracks cached parameters (explosion power, `isDangerous()` state, owner attribution, block-breaking capability). If a parameter changes or a block update occurs near the path, it recalculates immediately.
