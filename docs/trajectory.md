...
- **Asynchronous Execution Split & Preliminary Rendering**: Calculates predictions in two distinct phases for both initial projectile spawns and in-flight refreshes:
  - **Simulation Phase (Main Thread - <0.05 ms)**: Performs the 200-tick flight path raycast and captures a thread-safe, bounded, sparse `BlockStateSnapshot` around the collision point via `BlockStateSnapshot.create` (skipping capture for non-block-breaking projectiles via `profile.breaksBlocks()` or when explosion volume exceeds `MAX_SNAPSHOT_BLOCKS = 65_536`). Immediately constructs a lightweight preliminary `PredictionData` (`TrajectoryPredictor.createPreliminaryPrediction`) containing the path and hit results so ribbon trails render on frame 0 with zero visual latency.
  - **Prediction Phase (Background Thread)**: Submits heavy calculations for the 1,352-ray broken blocks list (`ImpactPredictor.predictBrokenBlocks`) and procedural dome mesh quad generation (`TrajectoryPredictor.createRenderData`) to the background worker thread (`FireballPredictor-Worker`).
  - **Result Publishing**: Reactively publishes full `PredictionData` back to the main thread via `Minecraft.getInstance().execute(...)` once background computation completes.
...
- **Deduplicated Updates & Collision Caching**: Tracks an `isCalculating` flag for each active `TrackedPrediction` to prevent queueing redundant simulation tasks. Caches per-tick entity collision queries (`TrackedPrediction.getOrComputeDamageHit`) to eliminate redundant sweeps between `isThreateningPlayer` and the damage estimation overlay.
