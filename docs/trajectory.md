...
- **Dynamic Recalculation Cache Invalidation**: Tracks the projectile properties (such as the cached explosion power and `isDangerous()` states) used in the last successful prediction calculation. If a mismatch is detected, it schedules an immediate recalculation.
