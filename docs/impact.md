...
3. **Tier 3 (Per-Owner Dynamic Inference with TTL & Vanilla Mob/Dispenser Guard)**: Checks for an active, unexpired inferred power in `OWNER_INFERENCES` keyed by `ProjectileOwner` (e.g. `PLAYER`, `GHAST`, `COMMAND`, `UNKNOWN`), refreshed whenever an active shot of that owner type is observed (90-second default TTL). Known standard vanilla sources (`owner.isMob() || owner == ProjectileOwner.DISPENSER`) without custom inferences default to vanilla `1.0F`, preventing custom player blasts (such as power-3 Bedwars fireballs) from poisoning ghast or dispenser fireball dome sizes.
