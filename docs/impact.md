...
- **Dynamic Explosion Inference**: On minigame servers without server-side mod sync, [ClientPlayNetworkHandlerMixin.java](file:///c:/Users/simon/Documents/Programming/MinecraftModding/FireballPredictor/src/main/java/com/simonconrad/fireballpredictor/mixin/ClientPlayNetworkHandlerMixin.java) intercepts incoming `ExplosionS2CPacket` packets. To prevent duplicate execution from `NetworkThreadUtils.forceMainThread` and avoid wrong-thread crashes, it filters for main-thread execution (`client.isOnThread()`) and delegates to [ExplosionInferenceHandler.java](file:///c:/Users/simon/Documents/Programming/MinecraftModding/FireballPredictor/src/main/java/com/simonconrad/fireballpredictor/client/network/ExplosionInferenceHandler.java).
...
- **Entity Type Isolation**: `FireballInferenceTracker.isFireball` explicitly restricts power inference to standard fireball entities (`FireballEntity`, `SmallFireballEntity`, `DragonFireballEntity`), ensuring complete isolation from Wither Skulls and Wind Charges.
