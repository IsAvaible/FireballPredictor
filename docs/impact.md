...
### 3. Asynchronous Execution & Bounded Snapshot Caching
To prevent game micro-stutters, frame spikes, and memory pressure when predicting fireballs:
- **[BlockStateSnapshot.java](../src/main/java/com/simonconrad/fireballpredictor/math/BlockStateSnapshot.java)**: When a collision is predicted on the main thread, a thread-safe local block state snapshot is captured inside the bounding box of the explosion. It implements `BlockGetter` (`BlockView`) and stores sparse, null-coalescing references to non-air `BlockState` and non-empty `FluidState` instances to minimize memory and GC pressure.
- **Bounded Allocations & Power Clamping**: Snapshot dimensions are clamped to `MAX_SNAPSHOT_POWER = 9.0f` and bounded by world height limits (`getMinY()` to `getMaxY()`). If the total bounding box volume exceeds `MAX_SNAPSHOT_BLOCKS = 65_536` blocks, snapshot creation is skipped (`null`) to completely prevent cubic memory allocation spikes and main-thread hitching on extreme or malformed explosion powers.
- **Non-Destructive Projectile Bypass**: Projectiles that do not break blocks (e.g. wind charges and dragon/small fireballs via `profile.breaksBlocks()`) skip snapshot capturing entirely on the main thread.
- **Graceful Degradation**: If snapshot creation is skipped due to volume limits, block destruction predictions gracefully degrade to an empty list (`List.of()`) while flight trajectory ribbons, visual hit markers, shockwave dome meshes, damage calculations, and knockback estimates remain 100% functional.
