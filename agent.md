...
* [ImpactPredictor.java](src/main/java/com/simonconrad/fireballpredictor/math/ImpactPredictor.java): Replicates the vanilla explosion raycasting algorithm deterministically using custom config multipliers across 1352 outer boundary rays ($16^3 - 14^3$). Derived from `ProjectileProfile` to resolve static/dynamic blast power and short-circuit block destruction for wind charges (`List.of()`). Handles `WitherBoss.canDestroy` blast-resistance capping (`0.8F`) for charged wither skulls.
...
* [BlockStateSnapshot.java](src/main/java/com/simonconrad/fireballpredictor/math/BlockStateSnapshot.java): Thread-safe, bounded, and sparse local snapshot of block and fluid states captured on the main thread for background worker raycasting. Enforces a maximum volume limit (`MAX_SNAPSHOT_BLOCKS = 65_536`) and power caps (`MAX_SNAPSHOT_POWER = 9.0f`) with sparse null-coalescing arrays to eliminate cubic allocations and prevent frame spikes on extreme explosion powers.
...
* [ClientPowerLookup.java](src/main/java/com/simonconrad/fireballpredictor/client/network/ClientPowerLookup.java): 5-tier power resolution router (`ClientPowerCache` -> `serverFallbackPowers` -> unexpired per-owner inference -> standard vanilla source mob/dispenser fallback -> session fallback / `globalFallbackFireballPower`).
* [ExplosionInferenceHandler.java](src/main/java/com/simonconrad/fireballpredictor/client/network/ExplosionInferenceHandler.java): Infers fireball explosion power from incoming `ClientboundExplodePacket` radii or destroyed block spread/count, attributing power to `ProjectileOwner` with 90s TTL.
