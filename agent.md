...
* [DynamicHeartOverlayManager.java](src/main/java/com/simonconrad/fireballpredictor/client/render/DynamicHeartOverlayManager.java): Synthesizes dynamic 9x9 pixel-art cracking overlays on resource reload (`SimpleSynchronousResourceReloadListener`). Samples the active resource pack's heart sprite alpha silhouette and masks a 9x9 master crack network that extends across the full 7x7 icon bounds. Guarantees that square, circular, or custom health icons are cracked edge-to-edge with authentic Minecraft 1-to-1 pixel sizing, while vanilla hearts remain 100% identical to the original texture. Registers synthesized textures with `TextureManager` as `DynamicTexture` objects.
