...
### 2. Client Logic
* [FireballPredictorClient.java](src/main/java/com/simonconrad/fireballpredictor/client/FireballPredictorClient.java): Handles client ticks, owner-filtered tracking (fireballs, wither skulls, wind charges), updates prediction data, triggers ambient particles, manages block breaking overlays, evaluates player threat levels (`isThreateningPlayer`), ranks incoming threats to compute `currentDamageEstimate`, and registers HUD rendering hooks (`VanillaHudElements.CHAT` for impact warning badge and `VanillaHudElements.HEALTH_BAR` for cracking hearts).
...
* Modular Preview Rendering Pipeline (`com.simonconrad.fireballpredictor.client.gui.preview`):
  ...
  - [TrajectoryRenderer.java](src/main/java/com/simonconrad/fireballpredictor/client/gui/preview/TrajectoryRenderer.java): Draws animated 2D trajectory path with width, color, pulse, core glow, and style (`SOLID`/`DASHED`/`CORE_ONLY`).
  - [ShockwaveRenderer.java](src/main/java/com/simonconrad/fireballpredictor/client/gui/preview/ShockwaveRenderer.java): Draws animated 3x3 block grid, shockwave dome disc, Fresnel rim shading, and crack overlays.
  ...
  - [RenderUtils.java](src/main/java/com/simonconrad/fireballpredictor/client/gui/preview/RenderUtils.java): Color math, interpolation, and alpha blending utilities.

### 6. Automated Testing (GameTest)
* [FireballPredictorGameTest.java](src/main/java/com/simonconrad/fireballpredictor/gametest/FireballPredictorGameTest.java): Comprehensive 39-scenario regression test suite checking predicted trajectories, block destruction against real explosions, owner inference, server restriction masks, and the full explosion damage estimation pipeline (naked damage equality, armor & blast protection mitigation, line-of-sight cover reduction, direct hits for Large Fireballs & Wither Skulls, absorption heart clamping, trajectory-vs-damage hit separation, live entity movement re-querying, and warning badge type mapping).
