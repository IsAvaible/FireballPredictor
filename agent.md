...
* [FireballPredictor.java](src/main/java/com/simonconrad/fireballpredictor/FireballPredictor.java): Root server/mod entrypoint. Syncs fireball size/power (`FireballPowerPayload`) and authoritative owner info (`FireballOwnerPayload`) to clients. Pushes server tracking restrictions (`TrackingRulesPayload` from `ServerConfig.disabledOwnerMask()`) and mobGriefing state (`MobGriefingPayload`) to players on join, with all server network packet transmissions strictly guarded by `ServerPlayNetworking.canSend(...)` to prevent unmodded vanilla client disconnects. Registers `/fireballpredictor reload` (game-master permission) to reload server config and re-broadcast restrictions live.
...
### 2. Client Logic
* [FireballPredictorClient.java](src/main/java/com/simonconrad/fireballpredictor/client/FireballPredictorClient.java): Handles client ticks, owner-filtered tracking (fireballs, wither skulls, wind charges), runs synchronous unified prediction pipelines (generating trajectory ribbons, 1,352-ray explosion calculations, and dome meshes directly on the client tick), optimizes path checks with immediate 5-tick lookahead, checks collision shape emptiness (`getCollisionShape(world, hitPos).isEmpty()`) for fluid/replacement path invalidation, caches per-tick entity collisions (`getOrComputeDamageHit`), triggers ambient particles, manages block breaking overlays using reusable buffers to eliminate per-tick allocations, evaluates player threat levels (`isThreateningPlayer`), ranks incoming threats (tie-breaking equal damage by knockback velocity) to compute `currentDamageEstimate`, and registers HUD rendering hooks (`VanillaHudElements.CHAT` for impact warning badge and `VanillaHudElements.HEALTH_BAR` for cracking hearts) along with client resource reload listeners for preview caching.
* [TrackedProjectile.java](src/main/java/com/simonconrad/fireballpredictor/client/tracking/TrackedProjectile.java): Per-entity prediction tracking container. Evaluates `ModConfig` owner/type filters and `ServerTrackingRules` restriction masks, preserves player deflection ownership (`ProjectileOwner.PLAYER`) and deflector entity when receiving server sync packets, tracks cached power / charging states to invalidate predictions, and manages bounding box lifecycles.
* [ProjectileOwner.java](src/main/java/com/simonconrad/fireballpredictor/tracking/ProjectileOwner.java): Enum of inferred projectile origins (`BLAZE`, `GHAST`, `ENDER_DRAGON`, `WITHER`, `BREEZE`, `PLAYER`, `DISPENSER`, `COMMAND`, `UNKNOWN`).
...
* [ClientOwnerCache.java](src/main/java/com/simonconrad/fireballpredictor/client/tracking/ClientOwnerCache.java): Client cache for `FireballOwnerPayload` with update listener for in-flight upgrades. Provides centralized `resolveOwner(AbstractHurtingProjectile)` consolidation across client networking and tracker classes. Pure cache storage kept side-safe from client networking types; cleared on disconnect.
...
* [FireballOwnerPayload.java](src/main/java/com/simonconrad/fireballpredictor/network/FireballOwnerPayload.java): Server→client packet syncing owner type enum name (`ownerName`) + owner entity id via `ByteBufCodecs.STRING_UTF8`.
...
### 3. Projectile Profiling & Registry
* [ProjectileKind.java](src/main/java/com/simonconrad/fireballpredictor/projectile/ProjectileKind.java): Canonical enum of supported hurting projectile kinds (`LARGE_FIREBALL`, `SMALL_FIREBALL`, `DRAGON_FIREBALL`, `WITHER_SKULL`, `WIND_CHARGE`, `BREEZE_WIND_CHARGE`).
* [ProjectileProfile.java](src/main/java/com/simonconrad/fireballpredictor/projectile/ProjectileProfile.java): Immutable record encapsulating per-kind physics parameters (air/water drag, dangerous drag), explosion power rules (static vs dynamic), direct hit damage, block breaking ability, filter keys, warning categories, and default theme colors.
* [VanillaProfiles.java](src/main/java/com/simonconrad/fireballpredictor/projectile/VanillaProfiles.java): Central side-neutral registry mapping live `AbstractHurtingProjectile` entities to their `ProjectileProfile` and `ProjectileKind`, eliminating scattered `instanceof` branching across the mod.
* [ProjectileFilterKey.java](src/main/java/com/simonconrad/fireballpredictor/projectile/ProjectileFilterKey.java): Coarse filter enum mapping projectile kinds onto user config toggles (`FIREBALL`, `WITHER_SKULL`, `WIND_CHARGE`).
* [WarningProjectileType.java](src/main/java/com/simonconrad/fireballpredictor/projectile/WarningProjectileType.java): Side-neutral enum modeling projectile categories for the HUD impact warning badge (`FIREBALL` with `Items.FIRE_CHARGE`, `WITHER_SKULL` with `Items.WITHER_SKELETON_SKULL`, `WIND_CHARGE` with `Items.WIND_CHARGE`, and `DRAGON_FIREBALL` with `textures/entity/enderdragon/dragon_fireball.png`) alongside themed progress bar colors (including purple for dragon fireballs).

### 4. Math & Logic Simulators
* [TrajectoryPredictor.java](src/main/java/com/simonconrad/fireballpredictor/math/TrajectoryPredictor.java): Simulates projectile kinematics, raycasting, and profile-driven drag (`0.95` for fireballs in air, `0.73` for charged skulls in air, `0.8` for fireballs/skulls in water, `1.0` for wind charges). Distinguishes visual world collision (`hitResult` which continues past entities to display full paths on terrain) from entity collision (`damageHitResult`). Evaluates `canHitEntity` using `ProjectileAccessor`, provides `findDamageHitResult` to dynamically detect moving entities intersecting the remaining flight path on each tick, and `computeTrajectoryDomeIntercept` to calculate the exact incoming trajectory penetration vector on the shockwave dome sphere.
* [DamageCalculator.java](src/main/java/com/simonconrad/fireballpredictor/math/DamageCalculator.java): Client-safe simulation of Minecraft 26.2's explosion damage and knockback calculation. Evaluates deterministic line-of-sight exposure (`getSeenPercent`), profile-driven direct hit damage additions (6.0 for Large Fireballs, 5.0 for Small Fireballs, 8.0 for Wither Skulls), difficulty scaling, armor absorption (`CombatRules.getDamageAfterAbsorb`), Resistance effects, and client-side EPF computation (`getEnchantmentProtection` evaluating `EnchantmentEffectComponents.DAMAGE_PROTECTION` with equipment slot matching `holder.value().matchingSlot(slot)` and `CompositeLootItemConditionAccessor`), plus clamped non-negative knockback impulse magnitude (`computeKnockback`).
* [ImpactPredictor.java](src/main/java/com/simonconrad/fireballpredictor/math/ImpactPredictor.java): Replicates the vanilla explosion raycasting algorithm deterministically using custom config multipliers across 1352 outer boundary rays ($16^3 - 14^3$). Directly queries `ClientLevel` block and fluid states on the main thread, enforces a `power <= 50.0f` safety cap, defensively checks `profile == null`, and short-circuits block destruction for non-breaking projectiles (`List.of()`). Handles `WitherBoss.canDestroy` blast-resistance capping (`0.8F`) for charged wither skulls.
...
* [ClientPowerCache.java](src/main/java/com/simonconrad/fireballpredictor/client/network/ClientPowerCache.java): Pure cache storage encapsulating tracked entity powers client-side.
* [ClientPowerLookup.java](src/main/java/com/simonconrad/fireballpredictor/client/network/ClientPowerLookup.java): 4-tier power resolution router (`ClientPowerCache` -> `serverFallbackPowers` -> unexpired per-owner inference -> standard vanilla source mob/dispenser fallback -> `globalFallbackFireballPower`) with per-owner TTL-decayed inferences without cross-pollution, discrete canonical integer/half-integer snapping (`snapToCanonicalPower`), and multi-sample Exponential Moving Average (EMA, $\alpha = 0.65$) smoothing.
* [ExplosionInferenceHandler.java](src/main/java/com/simonconrad/fireballpredictor/client/network/ExplosionInferenceHandler.java): Infers fireball explosion power from incoming `ClientboundExplodePacket` radii or destroyed block count/spread, attributing power to `ProjectileOwner` with 180s (3 min) TTL. Consumes matched fireball records via `FireballInferenceTracker.consumeNearbyFireball` to prevent secondary nearby explosions from corrupting learned power. Validates bidirectional packet radius divergence (`estimatedBlockPower < radius * 0.75f || estimatedBlockPower > radius * 1.35f`) to reject inflated or deflated dummy packet radii, with air/water explosion safety (`estimatedBlockPower > 0.0f`). Normalizes broken block counts via $3\times3\times3$ local terrain solid angle enclosure ($f_{\text{solid}}$) and contact material blast resistance ($\bar{B}$).
* [FireballInferenceTracker.java](src/main/java/com/simonconrad/fireballpredictor/client/network/FireballInferenceTracker.java): Side-safe tracker managing `entityId`, `owner`, `lastPos`, and `hitPos` for fireballs with 3.0-block radius matching, active TTL touch refreshing, proactive expiration pruning, single-use record consumption (`consumeNearbyFireball`), and fallback location recording in `recordFinalFireballLocation` for untracked or long-distance flights. Includes explicit `isFireball` classification.
* [ClientPacketListenerMixin.java](src/main/java/com/simonconrad/fireballpredictor/mixin/ClientPacketListenerMixin.java): Intercepts `ClientboundExplodePacket` on main render thread (`client.isSameThread()`) and delegates to `ExplosionInferenceHandler` with active `client.level`.
...
* [LargeFireballMixin.java](src/main/java/com/simonconrad/fireballpredictor/mixin/LargeFireballMixin.java): Mixin implementing `FireballEntityAccessor` to dynamically sync power modifications/NBT loads to tracking clients, guarded by `ServerPlayNetworking.canSend(...)`.
* [GameRulesMixin.java](src/main/java/com/simonconrad/fireballpredictor/mixin/GameRulesMixin.java): Mixin broadcasting `MobGriefingPayload` on mob griefing rule changes, guarded by `ServerPlayNetworking.canSend(...)`.

### 6. Client Rendering & Compatibility
...
* [PredictionRenderer.java](src/main/java/com/simonconrad/fireballpredictor/client/render/PredictionRenderer.java): Draws the translucent trajectory ribbon and shockwave dome using `PredictionPipelines.PREDICTION` with entity-specific colors and visual theme overrides, submitting trail and dome with decoupled depth-sorting keys (camera distance to `fireball.position()` vs `hitPos`) to eliminate translucent sorting artifacts. Renders HUD impact warning badge with `impactBadgePosition` alignment helper.
* [PredictionFeatureRenderer.java](src/main/java/com/simonconrad/fireballpredictor/client/render/PredictionFeatureRenderer.java): Feature renderer emitting standard 3D trajectory ribbons (outer shroud + inner core) and procedural Fresnel shockwave domes. Delegates theme passes and overlays to `PredictionThemeRenderer`, passing each segment's cumulative arc length (blocks from the trajectory start) so trail decorations can anchor to world-space distance instead of the per-tick segment index.
* [PredictionThemeRenderer.java](src/main/java/com/simonconrad/fireballpredictor/client/render/PredictionThemeRenderer.java): Handles all 3D world-space visual theme passes, animated particle/glyph overlays, and procedural geometry generation (lightning arcs radiating from trajectory entry intercepts, soul tendrils, flames, code rain, black hole accretion disk/void, cherry blossoms, arcade sprites, fighter jets). Trail decorations are anchored to cumulative arc length rather than the per-tick path segment index, so their world-space size and spacing stay identical on long/fast trajectories while the element count scales with path length.
* [HeartOverlayRenderer.java](src/main/java/com/simonconrad/fireballpredictor/client/render/HeartOverlayRenderer.java): Draws the "cracking fireball hearts" overlay on top of the vanilla health bar via `HudElementRegistry.attachElementAfter(VanillaHudElements.HEALTH_BAR, ...)`. Allocates damage to absorption first and then current health, rendering silhouette-adaptive 9x9 pixel-art crack overlays via `DynamicHeartOverlayManager` (with fallback to bundled static sprites). Also renders the text damage/knockback readout (`-X.X❤  ⚡Y.Yb/s`) positioned relative to the impact badge, or anchored flush with the health bar when the badge is hidden.
* [DynamicHeartOverlayManager.java](src/main/java/com/simonconrad/fireballpredictor/client/render/DynamicHeartOverlayManager.java): Synthesizes dynamic 9x9 pixel-art cracking overlays on resource reload (`SimpleSynchronousResourceReloadListener`). Samples the active resource pack's heart sprite alpha silhouette and masks a 9x9 master crack network that extends across the full 7x7 icon bounds (including dedicated Thermal Scorch frozen master textures). Strips dark outer borders via `HeartMaskHelper` and cross-references `container.png` to protect custom resource pack border outlines. Manages leak-safe off-heap `NativeImage` lifecycles via top-level disposal. Registers synthesized textures with `TextureManager` as `DynamicTexture` objects.
* [HeartMaskHelper.java](src/main/java/com/simonconrad/fireballpredictor/client/render/HeartMaskHelper.java): Environment-agnostic utility for 9x9 heart icon silhouette extraction, standard 20-pixel outer border coordinate detection, and dark border pixel filtering (`isDarkBorderPixel`) to protect custom resource pack health borders.
* [ThemePreviewGallery.java](src/main/java/com/simonconrad/fireballpredictor/client/render/ThemePreviewGallery.java): Circular 3D exhibition gallery spawning all 16 visual themes simultaneously with raycast click-selection, line-of-sight targeting, and decoupled translucent depth sorting for dome and trajectory trail.
...
* Modular Preview Rendering Pipeline (`com.simonconrad.fireballpredictor.client.gui.preview`):
  ...
  - [DamageEstimatorRenderer.java](src/main/java/com/simonconrad/fireballpredictor/client/gui/preview/DamageEstimatorRenderer.java): Renders animated cracking damage hearts using dynamic master-synthesized textures on a 10-heart health bar with rising fiery embers, and the impact badge with damage/knockback readout.
...
### 7. Automated Testing (GameTest)
* Dedicated GameTest source set (`src/gametest/java` and `src/gametest/resources/fabric.mod.json`) split across [`TrajectoryTests.java`](src/gametest/java/com/simonconrad/fireballpredictor/gametest/TrajectoryTests.java), [`DamageTests.java`](src/gametest/java/com/simonconrad/fireballpredictor/gametest/DamageTests.java), [`OwnerTests.java`](src/gametest/java/com/simonconrad/fireballpredictor/gametest/OwnerTests.java), [`ThemeTests.java`](src/gametest/java/com/simonconrad/fireballpredictor/gametest/ThemeTests.java), and [`HeartTextureTests.java`](src/gametest/java/com/simonconrad/fireballpredictor/gametest/HeartTextureTests.java) (sharing [`GameTestBase.java`](src/gametest/java/com/simonconrad/fireballpredictor/gametest/GameTestBase.java) with `resetGlobalState()`): Comprehensive regression test suite of 78 automated GameTests checking predicted trajectories, trajectory dome boundary intercepts, block destruction against real explosions, owner inference, server restriction masks, the visual theme roster, zero-allocation color math pipeline, preview gallery procedural dome meshes, power-cache sentinel handling (server `-1.0f` fallthrough), theme time/color pins (frozen pulse factor, DEFAULT core color), extreme-power snapshot safety & graceful degradation, non-breaking projectile snapshot bypass, sparse state retrieval, the full explosion damage estimation pipeline with equipment-slot EPF matching, and heart overlay texture dimensions, half-unit transparency constraints, master crack coverage, border outline detection, and dynamic synthesis mask accuracy.

### 8. Build & Publishing Infrastructure
