...
* Modular Preview Rendering Pipeline (`com.simonconrad.fireballpredictor.client.gui.preview`):
  ...
  - [ShockwaveRenderer.java](src/main/java/com/simonconrad/fireballpredictor/client/gui/preview/ShockwaveRenderer.java): Draws animated 3x3 block grid, shockwave dome disc via banded horizontal scanlines, Fresnel rim shading, and crack overlays.
...
* [FireballPredictorGameTest.java](src/main/java/com/simonconrad/fireballpredictor/gametest/FireballPredictorGameTest.java): Comprehensive 41-scenario regression test suite checking predicted trajectories, block destruction against real explosions, owner inference, server restriction masks, and the full explosion damage estimation pipeline (naked damage equality, armor & blast protection mitigation, line-of-sight cover reduction, direct hits for Large Fireballs & Wither Skulls, absorption heart clamping, trajectory-vs-damage hit separation, live entity movement re-querying, wind charge 0-damage knockback estimation, and warning badge type mapping).
