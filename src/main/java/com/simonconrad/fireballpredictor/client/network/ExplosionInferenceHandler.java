...
public class ExplosionInferenceHandler {
    ...
    public static void onExplosion(Vec3d explosionPos, float radius, List<BlockPos> affectedBlocks) {
        onExplosion(explosionPos, radius, affectedBlocks != null ? affectedBlocks.size() : 0, affectedBlocks);
    }

    public static void onExplosion(Vec3d explosionPos, float radius, int blockCount, List<BlockPos> affectedBlocks) {
        ...
        if (affectedBlocks != null && !affectedBlocks.isEmpty()) {
            ...
            for (BlockPos pos : affectedBlocks) {
                ...
                double distSq = explosionPos.squaredDistanceTo(blockCenter);
                if (distSq > maxDistSq) {
                    maxDistSq = distSq;
                }
            }
