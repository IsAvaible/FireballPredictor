...
final class HudRenderer {
    ...
    static void render(Painter p, int x, int y, int w, int h,
        ...
        int scaledOffX = Math.round(MathHelper.clamp(offX * scale, -fw / 3.0f, fw / 3.0f));
        int scaledOffY = Math.round(MathHelper.clamp(offY * scale, -fh / 3.0f, fh / 3.0f));
        ...
        // Keep on-frame
        bx = MathHelper.clamp(bx, fx + 1, Math.max(fx + 1, fx + fw - badge - 1));
        by = MathHelper.clamp(by, fy + 1, Math.max(fy + 1, fy + fh - badge - 1));
        ...
        if (effectSpriteAvailable) {
            try {
                SpriteBlitter.draw(g, Identifier.ofVanilla("hud/effect_background"), bx, by, badge, badge);
        ...
        if (offX != 0 || offY != 0) {
            ...
            if (mc != null && mc.textRenderer != null) {
                ...
                int tx = MathHelper.clamp(bx + (badge - tw) / 2,
                        fx + 2, Math.max(fx + 2, fx + fw - tw - 2));
                int ty = MathHelper.clamp(by + badge + 2,
                        fy + 2, Math.max(fy + 2, fy + fh - mc.textRenderer.fontHeight - 1));
                g.drawText(mc.textRenderer, label, tx, ty, 0xFFB0B8C0, false);
            }
        }
    }
}
