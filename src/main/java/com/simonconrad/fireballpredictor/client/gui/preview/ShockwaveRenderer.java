...
final class ShockwaveRenderer {
    ...
    static void render(Painter p, int x, int y, int w, int h, boolean wind,
        ...
        for (int row = 0; row < grid; row++) {
            for (int col = 0; col < grid; col++) {
                ...
                if (showBlocks) {
                    ...
                    int crackA = MathHelper.clamp((int) ((40 + severity * 45) * flash), 0, 255);
        ...
        int maxY = MathHelper.ceil(cy + radius);
        float fresnelWeight = MathHelper.clamp(fresnelStrength, 0.0f, 1.0f);
        ...
        for (int py = minY; py <= maxY; py++) {
            ...
            int minX = MathHelper.floor(cx - maxDx);
            int maxX = MathHelper.ceil(cx + maxDx);
            ...
            for (int px = minX; px <= maxX; px++) {
                ...
                float modulatedAlpha = MathHelper.lerp(fresnel, 20.0f, 135.0f);
                float baseAlpha = MathHelper.lerp(fresnelWeight, flatAlpha, modulatedAlpha);

                int a = MathHelper.clamp((int) (baseAlpha * pulse), 0, 255);
        ...
        ring(p, cx, cy, radius, 1.6f,
                pack(r, g, b, MathHelper.clamp((int) (200 * pulse), 0, 255)));
        ring(p, cx, cy, radius * 0.72f, 1.0f,
                pack(r, g, b, MathHelper.clamp((int) (70 * pulse), 0, 120)));
    ...
    private static void ring(Painter p, float cx, float cy, float radius,
        ...
        int y0 = MathHelper.floor(cy - outer);
        int y1 = MathHelper.ceil(cy + outer);
