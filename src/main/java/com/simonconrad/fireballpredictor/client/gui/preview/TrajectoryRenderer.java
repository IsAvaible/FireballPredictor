...
final class TrajectoryRenderer {
    ...
    static void render(Painter p, int x, int y, int w, int h, boolean wind,
        ...
        float halfWidth = MathHelper.clamp(1.1f + widthFactor * 7.0f, 1.1f, 9.0f);
        ...
        int pxStart = MathHelper.ceil(left);
        int pxEnd = MathHelper.floor(right);
        ...
        if (cycle < TRAVEL) {
            // ProjectileEntity head riding the arc
    ...
    private static float widthProfile(float t) {
        float launch = MathHelper.clamp(t / 0.06f, 0.0f, 1.0f);
        float impact = MathHelper.clamp((1.0f - t) / 0.14f, 0.0f, 1.0f);
