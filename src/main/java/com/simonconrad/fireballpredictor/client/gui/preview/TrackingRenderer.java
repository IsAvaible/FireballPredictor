 ...
 *   <li>MobEntity-master overview: miniature row of hostile mobs + status plate</li>
 * </ul>
 */
final class TrackingRenderer {
    ...
    private static void renderLockOn(Painter p, int x, int y, int w, int h,
        ...
        int icon = MathHelper.clamp(Math.min(h - 8, w / 4), 10, 24);
        ...
        if (ex - sx >= 10f) {
            ...
            for (int px = MathHelper.ceil(sx); px <= MathHelper.floor(ex); px++) {
    ...
    private static void renderOverview(Painter p, int x, int y, int w, int h,
        ...
        if (effectSpriteAvailable) {
            try {
                SpriteBlitter.draw(g, Identifier.ofVanilla("hud/effect_background"), px, py, plate, plate);
        ...
        //     int tx = MathHelper.clamp(px + (plate - tw) / 2, x + 2, x + w - tw - 2);
        //     int ty = MathHelper.clamp(py + plate + 1, y + 1, y + h - mc.textRenderer.fontHeight - 1);
        //     // Only draw if it fits without colliding with the chip row
        //     if (ty + mc.textRenderer.fontHeight <= y + h - 2 && plate + 6 < h) {
        //         g.drawText(mc.textRenderer, text, tx, ty, enabled ? 0xFFB8C0C8 : 0xFF8A5050, false);
        //     }
        // }
        ...
        int n = chips.size();
        int chip = MathHelper.clamp(Math.min(h - 8, chipAreaW / Math.max(1, n) - 2), 8, 18);
        ...
        for (int i = 0; i < n; i++) {
            ...
            if (on) {
                ...
                int pr = MathHelper.clamp((int) (180 * pulse), 40, 220);
