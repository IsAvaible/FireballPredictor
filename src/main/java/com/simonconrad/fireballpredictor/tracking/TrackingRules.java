 ...
 * (Player / Dispenser / Command projectiles) - 1.8.9 port of master's TrackingRules.
 *
 * <p>Bit-mask based so a server can disable prediction tracking either for the whole
 * group or for individual sub-options. The mask is built from {@code ServerConfig},
 * transported via the {@code fireballpredictor} custom channel and enforced by the
 * client inside {@code FireballPredictorClient.shouldTrackOwner}.
 */
public final class TrackingRules {
     ...
     * server-restrictable (hostile mobs stay unrestricted). {@link ProjectileOwner#UNKNOWN}
     * shares the command sub-option.
     */
