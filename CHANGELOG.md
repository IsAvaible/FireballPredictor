...
## Unreleased
- **Status-Aware Frozen Heart Cracking Overlays:** Resolved complementary color clashing and edge bleed-through on frozen hearts by introducing dedicated status-aware overlay styles with 100% silhouette coverage (preventing underlying cyan ice pixels from leaking at the border or top cleft). Fully frozen players (`player.isFullyFrozen()`) render with either **Thermal Scorch** (opaque charred basalt borders with molten magma fissure cores and an incandescent thermal burst warning flash) or **Frostbite Shatter** (deep midnight navy fractures with sub-zero crystalline glints and glacial flash), configurable in the visual settings (`Frozen Hearts Damage Style`), while Poison and Wither hearts preserve their authentic warm color blending.
- **Dynamic Frozen Silhouette Masking:** The overlay synthesization pipeline now independently samples `minecraft:textures/gui/sprites/hud/heart/frozen_full.png` alongside the standard heart sprite, guaranteeing exact edge-to-edge silhouette conformity for custom resource packs that modify frozen heart shapes.
